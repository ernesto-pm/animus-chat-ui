...
import {createScenario, getAllUsers} from "~/services/openapi";
import {useLoaderData} from "@remix-run/react";
import {authenticate} from "~/services/auth.server";

export async function loader({ request }: LoaderFunctionArgs) {
    const user = await authenticate(request, "/new/scenario") // we specify that we want to return here
...
export default function NewScenario() {
    ...
    async function onSubmit(data: FormArguments) {
        ...
        const response = await createScenario({body: {
                name: data.name,
    ...
    return (
        <div className="p-10 flex flex-col space-y-2">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col space-y-5">
                ...
                <div className="flex flex-col space-x-1">
                    <div>Content:</div>
                    <textarea {...register("content")} className="p-2" rows={4}/>
                </div>
